...
| 항목                      | Phase 3<br/>(단일 인스턴스) | Phase 4<br/>(Scale-out) | Phase 5<br/>(Scale-out + PgBouncer) | Phase 6<br/>(쿼리 최적화) | Phase 4 효과 | Phase 5 효과 | Phase 6 효과 |
| ------------------------- | --------------------------- | ----------------------- | ----------------------------------- | ------------------------- | ------------ | ------------ | ------------ |
| **응답 시간**             |
| p50 latency               | 258 ms                      | 744 ms                  | 790 ms                              | 738.3 ms                  | +188% 악화   | +6.2% 악화   | -6.5% 개선   |
| p95 latency               | 10,714 ms                   | 4,431 ms                | 4,395 ms                            | 4,095.8 ms                | -58.6% 개선  | -0.8% 개선   | -6.8% 개선   |
| **처리량**                |
| RPS                       | 100.5 req/s                 | 712.4 req/s             | 713.0 req/s                         | 724.5 req/s               | +609% 증가   | +0.1% 증가   | +1.6% 증가   |
| 요청 실패율               | 1.22%                       | 2.02%                   | 2.00%                               | 2.00%                     | +65.6% 악화  | -1.0% 개선   | 변화 없음    |
| **리디렉션 지연**         |
| redirect_latency p(95)    | 154,583 ms                  | 9,239 ms                | 9,376 ms                            | 8,964 ms                  | -94.0% 개선  | +1.5% 악화   | -4.4% 개선   |
| **연결 관련**             |
| http_req_blocked p(95)    | 7,686 ms                    | 911 ms                  | 883 ms                              | 839.3 ms                  | -88.1% 개선  | -3.1% 개선   | -4.9% 개선   |
| http_req_connecting p(95) | 830 ms                      | 450 ms                  | 465 ms                              | 393.4 ms                  | -45.8% 개선  | +3.3% 악화   | -15.4% 개선  |

**참고**:

- 전체 요청 시간(`http_req_duration`)은 네트워크 지연을 포함합니다.
- Phase 3는 단일 인스턴스, Phase 4, 5, 6는 10개 인스턴스로 실행되었습니다.
...
쿼리 최적화는 전반적인 성능을 개선했으며, 특히 응답 시간에서 일관된 개선이 확인되었습니다. 하지만 요청 실패율과 p95 응답 시간은 여전히 목표를 달성하지 못했습니다. 이는 다른 성능 병목(애플리케이션 로직, 로깅 오버헤드 등)이 존재함을 시사합니다.
...
- **전체 요청 시간 (http_req_duration)**:
  ...
  - 평균: 1,310 ms → 1,252.6 ms (-4.4% 개선)
- **리디렉션 지연 (redirect_latency)**:
  - p95 latency: 9,376 ms → 8,964 ms (-4.4% 개선)
- **연결 블로킹 시간 (http_req_blocked)**:
  - p95 latency: 883 ms → 839.3 ms (-4.9% 개선)
- **처리량 (RPS)**: 713.0 → 724.5 (+1.6% 증가)
- **요청 실패율**: 2.00% → 2.00% (변화 없음)

---

## 종합 효과 (Phase 3 → Phase 6)

**전체 개선**:

- **처리량**:
  - RPS: 100.5 → 724.5 (+621% 증가)
- **응답 시간**:
  - p50 latency: 258 ms → 738.3 ms (+186% 악화)
  - p95 latency: 10,714 ms → 4,095.8 ms (-61.8% 개선)
  - 평균: 2,244 ms → 1,252.6 ms (-44.2% 개선)
- **리디렉션 지연**:
  - p95 latency: 154,583 ms → 8,964 ms (-94.2% 개선)
  - 평균: 30,903 ms → 3,346.5 ms (-89.2% 개선)
- **연결 블로킹 시간**:
  - p95 latency: 7,686 ms → 839.3 ms (-89.1% 개선)
- **요청 실패율**: 1.22% → 2.00% (+64.0% 악화)

**전략 조합의 효과**:

Scale-out, PgBouncer, 쿼리 최적화를 함께 사용하면:

- **Scale-out**: 처리량을 크게 향상시키고 p95 응답 시간을 개선
- **PgBouncer**: 연결 풀 경합을 해소하여 PostgreSQL 서버 부하 감소
- **쿼리 최적화**: DB 오버헤드 감소로 응답 시간 개선
...
3. **쿼리 최적화의 효과**
   ...
   - p50 응답 시간을 6.5% 개선 (790ms → 738.3ms)
   - p95 응답 시간을 6.8% 개선 (4,395ms → 4,095.8ms)
   - 처리량을 1.6% 증가 (713.0 → 724.5 req/s)
   - 엔티티 매핑 오버헤드 제거 및 네트워크 전송량 감소

4. **전략 조합의 중요성**
   - Scale-out은 처리량 향상에 필수적
   - PgBouncer는 Scale-out 시 연결 풀 경합 문제 해소에 필수적
   - 쿼리 최적화는 DB 오버헤드 감소로 응답 시간 개선에 기여
...
- [Phase 6: 쿼리 최적화](./phase6-query-optimization.md)
